...
void mainwindow::on_pushButton_5_clicked()                      //set speed through spinbox
{
  /* ui->curspeed->setText(ui->spinBox->cleanText());

   QString curspeed = ui->spinBox->cleanText();
   QTextStream str(&curspeed, QIODevice::ReadOnly);
   QString cmddst = str.readAll();
   QString cmdsrc = "echo SP";

   QString cmd = strcat(cmddst, cmdsrc);*/
   ...
   ui->curspeed->setText(ui->spinBox->cleanText());
   QString command="echo SP";
   QString curspeed = ui->spinBox->cleanText();
   qDebug() << curspeed;
   command.append(curspeed);
   command.append(" >> /dev/lp0");
   system((const char*)command.toStdString().c_str());
}
...
void mainwindow::on_pushButton_2_clicked()   //step-run forward
{
    /*QString text = ui->textEdit->toPlainText();
    QTextStream str(&text, QIODevice::ReadOnly);
